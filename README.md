...
**基于近红外rPPG的夜间非接触式疲劳预警技术**，三项可申请发明专利的技术方案：

1. **环境红外共模抑制 rPPG**（`rppg/nir_illum.py`）
   夜间对向车灯/路灯的 850nm 干扰会淹没微弱脉搏信号。本方案同步采样"前额灌注区"与"背景非灌注参考区"，以最小二乘估计共模增益并差分对消，恢复干净脉搏波——填补 RGB rPPG 无法用于夜间单通道近红外的空白。
2. **信号质量(SQI)驱动的置信度自适应融合**（`rppg/signal_quality.py` + `fusion/feature_fusion.py`）
   实时评估各模态可信度，动态调整融合权重、抑制瞬时失效模态，避免固定权重导致的误报/漏报。
3. **迟滞—持续性—多证据—自动恢复分级状态机**（`fusion/fatigue_classifier.py`）
   消除阈值抖动与报警疲劳的分级光电闭环预警。

基础能力：夜间完全可用（850nm 成像）、非接触、不影响驾驶（850nm 不可见）、多模态融合。

📄 **专利交底书**（含权利要求书、实施方式、附图说明）：[docs/专利交底书_夜间NIR疲劳预警系统.md](docs/专利交底书_夜间NIR疲劳预警系统.md)
🧪 **专利要点功能验证**：`python tests/test_patent_features.py`
...
"rppg_method": "NIR_CMR",       # 环境红外共模抑制算法（专利要点一）
"illum_rejection_enabled": True,# 启用参考区共模抑制
"adaptive_fusion_enabled": True,# 置信度自适应融合（专利要点二）
...
### 专利/知识产权
1. [专利交底书](docs/专利交底书_夜间NIR疲劳预警系统.md) - 权利要求书 + 实施方式 + 附图说明
2. `python tests/test_patent_features.py` - 专利要点功能性验证

### 配置调优
1. [config.py](python/config.py) - 配置文件
2. [疲劳判据参数手册](docs/疲劳判据参数手册.md) - 参数说明
...
### v3.1 (2026-07-06) - 产品化缺陷修复
- ✅ 修复相机实测帧率未参与 DSP 导致的**系统性心率偏差**
- ✅ 修复人脸丢失重捕获后**滑动缓冲时序断裂**导致的错误心率/HRV
- ✅ 修复自动恢复机制会在驾驶员**仍疲劳时误静默报警**的安全隐患
- ✅ 修复 NIR trend 模式 HRV 计算器每帧重建导致的**趋势判据失效**
- ✅ 融合/分级/串口统一为固定节律评估，稳定 EMA 与串口流量
- ✅ 头姿 solvePnP 仅在新 landmark 时重算，降低 ARM 负担
- ✅ 新增回归测试 `tests/test_bugfixes.py`，详见 [产品化缺陷修复记录](docs/产品化缺陷修复记录.md)

### v3.0 (2026-07-06) - 专利化技术升级
- ✅ 新增**环境红外共模抑制**算法 `NIR_CMR`（对消夜间车灯/路灯干扰）
- ✅ 新增**信号质量指数(SQI)驱动的置信度自适应融合**（抑制模态瞬时失效误报）
- ✅ 形成分级预警状态机的多证据/自动恢复防报警疲劳机制
- ✅ 输出发明专利交底书（权利要求书 + 实施方式 + 附图说明）
- ✅ 新增离线可复现的专利要点功能性验证 `tests/test_patent_features.py`
